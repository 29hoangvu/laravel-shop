...
<a href="{{ route('admindashboard.staff.create') }}" class="btn btn-primary mb-3">
            ...
            <tbody>
                ...
                @foreach ($staff as $member)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admindashboard.staff.edit', $member->staff_id) }}"
                                class="btn btn-sm btn-warning">
                                <i class="fa fa-edit"></i>
                            </a>
                            <button type="button" class="btn btn-sm btn-danger"
                                onclick="deleteStaff({{ $member->staff_id }})">
...
<script>
    ...
    function deleteStaff(id) {
        if (confirm('Bạn có chắc chắn muốn xóa nhân viên này?')) {
            $.ajax({
                url: '{{ route('admindashboard.staff.destroy', ':id') }}'.replace(':id', id),
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
